...
do
	...
	assets=$(jq -r  ".[] | select(.tag_name == \"${version}\") | .assets[] | select(.content_type != \"text/plain\") | select (.name | endswith(\".txt\") | not) | select (.name | endswith(\".bom\") | not) | select (.name | endswith(\"-src.tar.gz\") | not) | select (.name | endswith(\"-src-full.tar.gz\") | not) | select (.name | contains(\"-full-nosign\") | not) | .name" "${TEMP_DIR}/releases-${VENDOR}.json")
	for asset in ${assets}
	do
