...
download_file 'https://api.adoptium.net/v3/info/available_releases' "${RELEASES_FILE}"
AVAILABLE_RELEASES=$(jq '.available_releases[]' "${RELEASES_FILE}")

for release in ${AVAILABLE_RELEASES}
do
	page=0
	while download_file "https://api.adoptium.net/v3/assets/feature_releases/${release}/ga?page=${page}&page_size=20&project=jdk&sort_order=ASC&vendor=adoptium" "${TEMP_DIR}/release-${release}-${page}.json"
	do
		page=$((page+1))
	done
done
